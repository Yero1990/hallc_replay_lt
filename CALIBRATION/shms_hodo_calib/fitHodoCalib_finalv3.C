...
void fitHodoCalib_finalv3(TString filename,Int_t runNUM,Bool_t cosmic_flag=kFALSE)
{
...
//HARD CUT NOTICE
  ...
  //Fill each matrix element Ay (61rows,61cols) with the lambda coefficients
	...
	for (Int_t npl = 0; npl < PLANES; npl++ )
	  {
		...
		//cout << "mean_time_flg = " << mean_time_flg[i][npl][ipmt] << endl;
		...
		/*
		cout << "scin_time = " << scin_time << endl; 

		cout << "-----parameters----" << endl;
		cout << "npl, ipmt = " << npl << ", " << ipmt << endl;
		cout << "fTdc_Thrs = " << fTdc_Thrs << endl;
		cout << "fHodoPos_c2 = " << fHodoPos_c2[npl][ipmt] << endl;
		cout << "fHodoNeg_c2 = " << fHodoNeg_c2[npl][ipmt] << endl;
		cout << "fHodo_LCoeff " << fHodo_LCoeff[npl][ipmt] << endl;
		cout << "fHodoVelFit = " << fHodoVelFit[npl][ipmt] << endl;
		cout << "fHodoCableFit = " << fHodoCableFit[npl][ipmt] << endl;
		cout << "scin_sigma = " << scin_sigma << endl;
		cout << "-------------------" << endl;
		*/
		...
		/*
		cout << "scinWeight = " << printf("%f", scinWeight) << endl;
		cout << "sumW = "<< printf("%f", sumW) << endl;
		cout << "sumT = "<< printf("%f", sumT) << endl; 
		cout << "sumZ = "<< printf("%f", sumZ) << endl; 
		cout << "sumZZ= " << printf("%f", sumZZ) << endl; 
		cout << "sumTZ = " << printf("%f", sumTZ) << endl;
		cout << "sumW * sumZZ = " << printf("%f", (sumW * sumZZ)) << endl;
		cout << "sumZ * sumZ = " << printf("%f", (sumZ * sumZ)) << endl;  
	        */
		//cout << "sumZ * sumZ = " << (sumZ * sumZ) << endl;
		//printf("%f \n", sumW * sumZZ);
		//printf("%f \n", sumZ * sumZ);
	...
	if (  TMath::Abs(tmpDenom)  > ( 1 / 10000000000.0 ) ) {
	  ...
	  //cout << "plane = " << npl << endl;
	  //cout << "ipmt = " << ipmt << endl;
	  //cout << "timep, timen = " << timep << ", " << timen << endl;
	  
	  //cout << "zPosition = " << zPosition << endl;
	  //cout << "scin_time = " << scin_time << endl;
	  //cout << "scin_sigma = " << scin_sigma << endl;
	  //cout << "scinWeight = " << scinWeight << endl;
	  //cout << "sumW, sumT, sumZ, sumZZ, sumTZ = " << sumW << ", " << sumT << ", " << sumZ << ", " << sumZZ << ", " << sumTZ << endl;
	  //cout << "tmp = " << tmp << endl;
	  //cout << "tmpDenom = " << tmpDenom << endl;
	  //cout << "beta_calib (tmp/tmpDenom) = " << beta_calib << endl;
	  
	  //-----
	  /*
	    
	    
	    fPlanes[ip]->SetTrackXPosition(theTrack->GetX() + theTrack->GetTheta()*pl_zpos );  --> 'TrackXPos' that we read from TTree
	    fPlanes[ip]->SetTrackYPosition(theTrack->GetY() + theTrack->GetPhi()*pl_zpos );    --> 'TrackYPos' that we read from TTree
	    where:  pl_zpos+=fPlanes[ip]->GetZpos()+ (padind%2)*fPlanes[ip]->GetDzpos();  --> We define as z[PLANES][21] in the beginning of this code
	    
	    ** NOTE: From THaTrack.h (.cxx), 
	    x,y track pos, and GetX(), GetY(), we can reverse-engineer GetTheta(), GetPhi()
	    
	    cout << "x_fp     = " << fX     << " m"     << endl;  
	    cout << "y_fp     = " << fY     << " m"     << endl;
	    cout << "theta_fp = " << fTheta << " rad"   << endl; --> xp_fp
	    cout << "phi_fp   = " << fPhi   << " rad"   << endl; --> yp_fp
	    
	    Bottom Line:
	    I think that in Hall C Coordinates:  
	    theTrack->GetTheta() is actually xp_fp  (x' focal plane or dx/dz at focal plane) 
	    theTrack->GetPhi() is actually   yp_fp  (y' focal plane or dy/dz at focal plane) 
