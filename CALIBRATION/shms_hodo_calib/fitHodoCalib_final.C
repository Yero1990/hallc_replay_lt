...
void fitHodoCalib_final(TString filename,Int_t runNUM,Bool_t cosmic_flag=kFALSE)
{
  ...
  Int_t single_hit_cnt = 0;

  /*
  //C.Y. Jan 02, 2021 | add new branch
  TBranch *good_hod_1x_hit = T->Branch("good_hod_1x_hit", &good_hod_1x_nhits, "good_hod_1x_nhits/I");
  TBranch *good_hod_1y_hit = T->Branch("good_hod_1y_hit", &good_hod_1y_nhits, "good_hod_1y_nhits/I");
  TBranch *good_hod_2x_hit = T->Branch("good_hod_2x_hit", &good_hod_2x_nhits, "good_hod_2x_nhits/I");
  TBranch *good_hod_2y_hit = T->Branch("good_hod_2y_hit", &good_hod_2y_nhits, "good_hod_2y_nhits/I");
  */
  ...
  //----Histograms----
  TH1F *good_hod1x_nhits = new TH1F("good_hod1x_nhits", "Good Hodo 1X Hits", 100, 0, 25);
  TH1F *good_hod1y_nhits = new TH1F("good_hod1y_nhits", "Good Hodo 1Y Hits", 100, 0, 25);  
  TH1F *good_hod2x_nhits = new TH1F("good_hod2x_nhits", "Good Hodo 2X Hits", 100, 0, 25);  
  TH1F *good_hod2y_nhits = new TH1F("good_hod2y_nhits", "Good Hodo 2Y Hits", 100, 0, 25);  
  ...
  */
  ...
  Long64_t nentries = 10000; //T->GetEntries();
 ...
 //Loop over all entries
  ...
  for(Long64_t i=0; i<nentries; i++)
    {
      ...
      //good paddle hit counter of hodo planes (resets per entry) 
      good_hod_1x_nhits = 0;      
      good_hod_1y_nhits = 0;
      good_hod_2x_nhits = 0;
      good_hod_2y_nhits = 0;
      ...
      if(pid_pelec) // APPLY PID CUT TO SELECT CLEAN ELECTRONS
	{
      ...
      good_hod1x_nhits->Fill(good_hod_1x_nhits);
      good_hod1y_nhits->Fill(good_hod_1y_nhits); 
      good_hod2x_nhits->Fill(good_hod_2x_nhits); 
      good_hod2y_nhits->Fill(good_hod_2y_nhits); 
      */

      //Define good plane hit
      Bool_t good_plane_hit = kFALSE;
      good_plane_hit = (good_hod_1x_nhits==1) && (good_hod_1y_nhits==1) && (good_hod_2x_nhits==1) && (good_hod_2y_nhits==1); 

      cout << "ientry = " << i << endl;
      cout << "good_plane_hit ---- > " << good_plane_hit << endl;
      
      // ----------------- "LITE" HIT FILTERING (PART 2) -----------------
       if(pid_pelec && good_plane_hit)
	{
  ...
  c1->cd();
  good_hod1x_nhits->Draw();
  c1->SetLogy();
  c1->SaveAs("good_phod_1x_nhits.pdf");

  c2->cd();
  good_hod1y_nhits->Draw();  
  c2->SetLogy();
  c2->SaveAs("good_phod_1y_nhits.pdf"); 

  c3->cd();                                                                                                                                                  
  good_hod2x_nhits->Draw();                                                                                                                                             
  c3->SetLogy();
  c3->SaveAs("good_phod_2x_nhits.pdf");   

  c4->cd();                                                                                                                                                    
  good_hod2y_nhits->Draw();                                                                                                                                             
  c4->SetLogy();
  c4->SaveAs("good_phod_2y_nhits.pdf");   
  */
