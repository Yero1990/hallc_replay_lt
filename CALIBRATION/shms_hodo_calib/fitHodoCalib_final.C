	  ...
	  for (Int_t ipmt = 0; ipmt < maxPMT[npl]; ipmt++)
	       ...
	       if (side==0) //require ONLY one side, since a time diff between two pmts at each end is taken
		{
		  ...
		  h2Hist_TW_Corr_v_TrkPos[npl][ipmt]->GetXaxis()->CenterTitle();
	       ...
	       //T->SetBranchAddress(nDiffTWCorr,  &DiffDistTWCorr[npl]);  //C.Y. Jan 04, 2021 | This needs to be re-calculated using the updated hodo velocity/cable times from updated fits
  ...
  for(Long64_t i=0; i<nentries; i++)
      ...
      if(pid_pelec) // APPLY PID CUT TO SELECT CLEAN ELECTRONS
	{
      ...
      */
      ...
      if( (good_hod_1x_nhits==1) && (good_hod_1y_nhits==1) && (good_hod_2x_nhits==1) && (good_hod_2y_nhits==1) ) { single_hit_flg = kTRUE; } 
		  ...
		  for (Int_t ipmt = 0; ipmt < maxPMT[npl]; ipmt++)
		      ...
		      if(TdcTimeTWCorr[npl][0][ipmt] < 200. && TdcTimeTWCorr[npl][1][ipmt] < 200.)
			{
			  ...
			  if (side==0)  
				...
				} //end meantime +/- SIGMA CUT of TW Corr Time
	...
	} // end pid-electroc AND single plane hit CUTS (IMPORTANT ! ! ! )
       
    } //end entry loop

  //Update TTree with newly added branches
	...
	{	  
	  ...
	  for (Int_t ipmt = 0; ipmt < maxPMT[npl]; ipmt++)
	      ...
	      if (side==0)
		{
		  ...
		  //h2Hist_TWDiff_v_TrkPos[npl][ipmt]->Draw("colz");   // Jan 04, 2021 ** need to Re-locate ** (until after DiffDistTWCorr[npl][ipmt] is re-defined)
		  ...
		  //h1Hist_TWDiffTrkPos[npl][ipmt]->Draw();  // Jan 04, 2021 ** Re-locate ** (until after DiffDistTWCorr[npl][ipmt] is re-defined)
